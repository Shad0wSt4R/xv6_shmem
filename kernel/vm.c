...
copyuvm(pde_t *pgdir, uint sz, void** old_shmem_access, void** new_shmem_access)
{
  ...
  for(i=0;i<SHMEM_PAGES;i++){
    ...
    if(new_shmem_access[i] != NULL){
      shmemCount[i]++;
      if(mappages(d,new_shmem_access[i],PGSIZE,(uint)shmem_paddr[i],PTE_W|PTE_U)<0){
	goto bad;
      }
    }
  }
...
void* shmem_access(int pg_num){
  if(pg_num < 0 || pg_num >= SHMEM_PAGES){
    return NULL;
  }
  if(proc->shmems[pg_num] != 0){
